...
class MainActivity : ComponentActivity() {
    ...
    private lateinit var initiatePreferences: SharedPreferences

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        initiatePreferences =
            applicationContext.getSharedPreferences("initiate_preferences", MODE_PRIVATE)
        enableEdgeToEdge()
        setContent {
            ...
            NavHost(navController = navController, startDestination = MainRoute) {
                ...
                composable<ExpenditureSubmitRoute> { ExpenditureSubmitScreen(expenditureTagDao = expenditureTagDao) }
            }
        }
        ...
        lifecycleScope.launch {
            ...
            var dao = db.expenditureTagDao()
            // 第一次启动应用初始化操作
            if (!isFirstStartUp) {
                initExpenditureTags(EntryPointAccessors.fromActivity(this, DatabaseModule_UserDaoEntryPoint::class.java))
                initiatePreferences.edit().putBoolean("is_first_start_up", true).apply()
            }
        }
    }

    suspend fun initExpenditureTags(dao: ExpenditureTagDao){
        // 开支标签初始化
        ...
        dao.insertAll(*tags.toTypedArray())
    }
}